...
AES.prototype._get_public_key = function () {
    var that = this;
    this._request = getText(http, 'http://' + this._host + '/jdev/sys/getPublicKey', function (error, body) {
...
AES.prototype.dispose = function () {
    this._disposed = true;
...
AES.prototype._register_keyexchange_response = function() {
    ...
    this._api.command_chain.push({
        ...
        'callback': function(loxone_message) {
            var key = Buffer.from(that._decipher(loxone_message.value), 'hex').toString('utf8');

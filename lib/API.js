...
var API = function (host, username, password, permissions, info, reconnect, security, keepalive,
    ...
    this._reconnect_delay = typeof reconnect === 'number' && Number.isFinite(reconnect) && reconnect > 0
        ? reconnect : 1000;
...
API.prototype.register_connection = function (connection) {
    ...
    this.connection.on('connect', function () {
        that.emit('connect');
        if (that._abort || that.connection !== connection) return;
    ...
    this.connection.on('connection_error', function (error, reason) {
        ...
        if (that.connection === connection) that.reconnect(false, reason);
    });
    ...
    this.connection.on('message_event_table_text', function (messages) {
        messages.forEach(function (evt) {
            ...
            that.emit('update_event_text_' + evt.uuid.string, evt.text);
        });
    ...
    this.connection.on('message_event_table_daytimer', function (messages) {
        messages.forEach(function (evt) {
            ...
            that.emit('update_event_daytimer_' + evt.uuid.string, evt);
        });
    ...
    this.connection.on('message_event_table_weather', function (messages) {
        messages.forEach(function (evt) {
            ...
            that.emit('update_event_weather_' + evt.uuid.string, evt);
        });
...
API.prototype.reconnect = function (outOfService, reason) {
    var that = this;
    // A close listener may call abort(); let it change the stop flag without
    // recursively emitting close or creating another retry.
    if (that._reconnecting) return;
    that._reconnecting = true;
    try {
        clearTimeout(that._reconnect_timeout);
        that._reconnect_timeout = undefined;
        var connection = that.connection;
        that.connection = undefined;
        if (that._version_request) {
            that._version_request.destroy();
            that._version_request = undefined;
        }
        if (connection) {
        ...
        that._status_update_subscription = false;
        that.emit('close', outOfService === true, reason || 'reconnecting');
        if (that._abort) {
            that.emit('abort');
            return;
        }
        if (that._reconnect && that.connection === undefined) {
            that.emit('reconnect');
            if (that._abort || that.connection !== undefined) return;
            var retry = setTimeout(function () {
                if (that._reconnect_timeout !== retry) return;
                that._reconnect_timeout = undefined;
                if (!that._abort && that.connection === undefined) that.connect();
            }, that._reconnect_delay);
            that._reconnect_timeout = retry;
        }
    } finally {
...
API.prototype.send_command = function (command, secure, command_chain_entry) {
    ...
    if (secure) {
        ...
        var isFile = this.file_chain.some(function (item) {
            item.file.lastIndex = 0;
            return item.file.test(command);
        });
        this.connection.send(command, isFile ? command : undefined);
    }
};
...
API.prototype._message_text = function (message) {
    for (var index = this.command_chain.length - 1; index >= 0; index--) {
        var item = this.command_chain[index];
        item.control.lastIndex = 0;
...
API.prototype._message_file = function (message) {
    for (var index = this.file_chain.length - 1; index >= 0; index--) {
...
API.prototype.clear_auth_chain = function () {
    if (this._auth) {
        if (this._auth.dispose) this._auth.dispose();
...
API.prototype.register_auth_object = function () {
    ...
    this._auth.on('authorized', function () {
        if (that._abort) return;
        that.emit('authorized');
        if (that._abort || !that.connection) return;
...
API.prototype.perform_version_check = function (connection) {
    ...
    var that = this;
    this._version_request = getText(https, 'https://' + this._host + '/jdev/cfg/api', function (error, body) {
        if (that._abort || that.connection !== connection) return;
        that._version_request = undefined;
        var major;
        if (!error) {
            try {
                ...
                var api = typeof json.LL.value === 'string'
                    ? JSON.parse(json.LL.value.replace(/'/g, '"')) : json.LL.value;
                major = Number(api.version.split('.')[0]);
                if (!Number.isInteger(major) || major < 1) throw new Error('Invalid Miniserver version');
            } catch (parseError) {
                error = parseError;
            }
        }
        if (error) {
            that.emit('connect_failed', error, error.message);
            if (that.connection === connection) that.reconnect();
            return;
        }
        if (major >= 10) that._security = 'JWT';
        else if (major >= 9) that._security = 'Token-Enc';
